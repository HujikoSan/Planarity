...
from planarity import (
    ...
    get_crossing_edges,
    is_connected,
    generate_random_planar_graph,  # New imports
)
...
class TestLineIntersection(unittest.TestCase):
    ...
    def test_simple_intersection(self):
        ...
        p2, q2 = (0, 10), (10, 0)
        self.assertTrue(
            do_lines_intersect(p1, q1, p2, q2), "Simple intersection failed"
        )
    ...
    def test_collinear_overlapping(self):
        ...
        p2, q2 = (2, 0), (4, 0)
        self.assertTrue(
            do_lines_intersect(p1, q1, p2, q2), "Collinear overlapping (1) failed"
        )
        self.assertTrue(
            do_lines_intersect(p2, q2, p1, q1), "Collinear overlapping (2) failed"
        )
    ...
    def test_collinear_non_overlapping(self):
        ...
        p2, q2 = (3, 0), (5, 0)
        self.assertFalse(
            do_lines_intersect(p1, q1, p2, q2), "Collinear non-overlapping failed"
        )
    ...
    def test_collinear_touching_endpoint(self):
        ...
        p2, q2 = (2, 0), (4, 0)
        self.assertTrue(
            do_lines_intersect(p1, q1, p2, q2), "Collinear touching endpoint failed"
        )
    ...
    def test_parallel_no_intersection(self):
        ...
        p2, q2 = (0, 5), (10, 5)
        self.assertFalse(
            do_lines_intersect(p1, q1, p2, q2), "Parallel no intersection failed"
        )
    ...
    def test_sharing_endpoint_L_shape(self):
        ...
        p2, q2 = (5, 0), (5, 5)
        self.assertTrue(
            do_lines_intersect(p1, q1, p2, q2), "Sharing endpoint L-shape failed"
        )
    ...
    def test_segments_cross_at_endpoint_special_case(self):
        ...
        p2, q2 = (5, 5), (5, 0)
        self.assertTrue(
            do_lines_intersect(p1, q1, p2, q2), "Cross at endpoint (1) failed"
        )

        p1, q1 = (5, 0), (5, 5)
        p2, q2 = (0, 0), (10, 0)
        self.assertTrue(
            do_lines_intersect(p1, q1, p2, q2), "Cross at endpoint (2) failed"
        )
...
class TestGetCrossingEdges(unittest.TestCase):  # Renamed class
    def test_no_crossings_square(self):
        ...
        self.assertEqual(
            get_crossing_edges(vertices, edges),
            expected_crossings,
            "No crossings square failed",
        )
    ...
    def test_square_with_crossing_diagonals(self):
        ...
        self.assertEqual(
            get_crossing_edges(vertices, edges),
            expected_crossings,
            "Square with crossing diagonals failed",
        )
    ...
    def test_multiple_crossings_star(self):
        ...
        self.assertEqual(
            get_crossing_edges(vertices, edges),
            expected_crossings,
            "Multiple crossings star failed",
        )
    ...
    def test_edges_sharing_vertex_no_crossing(self):
        ...
        self.assertEqual(
            get_crossing_edges(vertices, edges),
            expected_crossings,
            "Edges sharing vertex (triangle) failed",
        )
        ...
        self.assertEqual(
            get_crossing_edges(vertices, edges),
            expected_crossings,
            "Edges sharing vertex (square) failed",
        )
    ...
    def test_no_edges(self):
        ...
        expected_crossings = set()
        self.assertEqual(
            get_crossing_edges(vertices, edges), expected_crossings, "No edges failed"
        )
    ...
    def test_collinear_edges_sharing_vertex_no_crossing(self):
        ...
        self.assertEqual(
            get_crossing_edges(vertices, edges),
            expected_crossings,
            "Collinear edges sharing vertex failed",
        )
    ...
    def test_collinear_edges_not_sharing_vertex_overlapping_crossing(self):
        ...
        self.assertEqual(
            get_crossing_edges(vertices, edges),
            expected_crossings,
            "Collinear overlapping non-adjacent failed",
        )
    ...
    def test_collinear_edges_not_sharing_vertex_separate_no_crossing(self):
        ...
        self.assertEqual(
            get_crossing_edges(vertices, edges),
            expected_crossings,
            "Collinear separate non-adjacent failed",
        )
...
class TestIsConnected(unittest.TestCase):
    def test_empty_graph(self):
        self.assertTrue(
            is_connected([], 0), "Empty graph (0 vertices) should be connected."
        )
    ...
    def test_two_vertices_no_edge(self):
        self.assertFalse(
            is_connected([], 2), "Two vertices, no edge, should be disconnected."
        )

    def test_two_vertices_with_edge(self):
        self.assertTrue(
            is_connected([tuple(sorted((0, 1)))], 2),
            "Two vertices with an edge should be connected.",
        )
    ...
    def test_disconnected_two_triangles(self):
        edges = [
            tuple(sorted((0, 1))),
            tuple(sorted((1, 2))),
            tuple(sorted((2, 0))),  # Triangle 1
            tuple(sorted((3, 4))),
            tuple(sorted((4, 5))),
            tuple(sorted((5, 3))),  # Triangle 2
        ]
        self.assertFalse(
            is_connected(edges, 6), "Two separate triangles should be disconnected."
        )
    ...
    def test_graph_with_isolated_vertex(self):
        ...
        self.assertFalse(
            is_connected(edges, 4),
            "Graph with an isolated vertex should be disconnected.",
        )
...
class TestGenerateRandomPlanarGraph(unittest.TestCase):
    def _run_graph_generation_test(self, n_vertices):
        print(
            f"Testing graph generation for n_vertices = {n_vertices}"
        )  # Added print for long tests
        vertices, edges = generate_random_planar_graph(n_vertices)

        self.assertEqual(
            len(vertices),
            n_vertices,
            f"Expected {n_vertices} vertices, got {len(vertices)}",
        )
        ...
        if n_vertices == 0:
            ...
            if m == 1:  # check content if edge count is correct
                self.assertIn(
                    tuple(sorted((0, 1))), edges, "Edge (0,1) missing for n=2"
                )
        elif n_vertices >= 3:
            ...
            if (
                n_vertices > 0
            ):  # proceed only if there are vertices to calculate degrees for
            ...
            if n_vertices == 3:
                self.assertEqual(
                    m, 3, f"n=3, expected 3 edges, got {m}"
                )  # Should be exactly a triangle
                if len(vertices) == 3:  # Ensure degrees list is valid before checking
                    self.assertTrue(
                        all(d == 2 for d in degrees),
                        f"n=3, expected all degrees to be 2, got {degrees}",
                    )
            elif n_vertices == 4:
                ...
                if len(vertices) == 4:
                    self.assertTrue(
                        all(d == 3 for d in degrees),
                        f"n=4, expected all degrees to be 3 (K4), got {degrees}",
                    )
            elif n_vertices >= 5:  # For n >= 5, check min degree is at least 3
                if (
                    len(vertices) == n_vertices and n_vertices > 0
                ):  # Check if degrees list is valid
                    self.assertTrue(
                        all(d >= 3 for d in degrees),
                        f"n={n_vertices}, expected all degrees >= 3, got {degrees}",
                    )
    ...
    def test_generate_n3(self):
        self._run_graph_generation_test(
            3
        )  # Triangulation: 3 edges. Target: max(2, int(0.8*3)=2) to 3. Range [2,3]
    ...
    def test_generate_n4(self):
        self._run_graph_generation_test(
            4
        )  # Triangulation: 6 edges. Target: max(3, int(0.8*6)=4) to 6. Range [4,6]

    def test_generate_n5(self):
        self._run_graph_generation_test(
            5
        )  # Triangulation: 9 edges. Target: max(4, int(0.8*9)=7) to 9. Range [7,9]

    def test_generate_n10(self):
        self._run_graph_generation_test(
            10
        )  # Triangulation: 24 edges. Target: max(9, int(0.8*24)=19) to 24. Range [19,24]

    def test_generate_n20(self):
        self._run_graph_generation_test(
            20
        )  # Triangulation: 54 edges. Target: max(19, int(0.8*54)=43) to 54. Range [43,54]
